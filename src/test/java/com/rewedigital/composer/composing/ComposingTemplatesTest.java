...
public class ComposingTemplatesTest {
	...
	private TemplateComposer makeComposer(final Client client) {
		...
		return new AttoParserBasedComposer(new ValidatingContentFetcher(client, Collections.emptyMap(), session),
				session, new ComposerHtmlConfiguration("include", "content", "data-rd-options", maxRecursionDepth));
	}
	...
	private Response<ByteString> contentResponse(final Status status, final String content, final String head, final String contentType) {
